 ...
 * <li>반드시 <b>PreparedStatement</b> + <b>?</b> 파라미터 바인딩 사용 (체크리스트 3-4 — SQL
 * Injection 방어)</li>
 * <li>Connection 은 <code>DBUtil.getConnection()</code> 으로 획득,
 * <b>try-with-resources</b> 권장</li>
 * <li>SQLException 은 그대로 throws 처리</li>
 * </ul>
 *
 * <p>
 * JDBC 4단계 (체크리스트 3-3 — 절대 빠뜨리지 말 것): ① Driver Load (DBUtil 에서 처리됨) → ②
 * Connection 획득 → ③ PreparedStatement 작성 → ④ executeUpdate() / executeQuery() +
 * ResultSet 처리
 * </p>
 *
 * <p>
 * 필요 import (추가): <code>java.sql.Connection</code>,
 * <code>java.sql.PreparedStatement</code>, <code>java.sql.ResultSet</code>,
 * <code>java.util.ArrayList</code>
 * </p>
 *
 * <p>
 * 📌 SQL 문은 이미 상수로 선언되어 있음 — 그대로 사용 (직접 작성 금지).
 * </p>
 */
public class BookDAO {

	private static final String SQL_INSERT = "INSERT INTO Book_tbl (Book_code, Classification_Id, author, name, publisher, isreserve) "
			+ "VALUES (?, ?, ?, ?, ?, ?)";

	private static final String SQL_SELECT_ALL = "SELECT Book_code, Classification_Id, author, name, publisher, isreserve "
			+ "FROM Book_tbl " + "ORDER BY Book_code";

	private static final String SQL_UPDATE = "UPDATE Book_tbl "
			+ "   SET Classification_Id = ?, author = ?, name = ?, publisher = ?, isreserve = ? "
			+ " WHERE Book_code = ?";

	private static final String SQL_DELETE = "DELETE FROM Book_tbl WHERE Book_code = ?";
	 ...
	 * setObject 로 6개 파라미터 바인딩 → executeUpdate() 반환.
	 * </p>
	 */
	 ...
	 * rs.next() 루프 → Book 객체 채워서 List 에 추가.
	 * </p>
	 *
	 * <p>
	 * 주의: <code>Classification_Id</code> 는 null 가능 컬럼. <code>rs.getInt(...)</code>
	 * 후 <code>rs.wasNull()</code> 로 null 체크 필요.
	 * </p>
	 */
	public List<Book> selectAll() throws SQLException {
			...
			)
		{
			...
			while(rs.next()) {
				...
				m.setIsReserve(rs.getString("isreserve"));
				list.add(m);
			}
	 ...
	 *
	 * <p>
	 * 학생 TODO: SQL_UPDATE 실행. 마지막 파라미터(?)는 WHERE 의 Book_code.
	 * </p>
	 */
	 ...
	 *
	 * <p>
	 * 학생 TODO: SQL_DELETE 실행. ?에 bookCode 바인딩.
	 * </p>
	 */
